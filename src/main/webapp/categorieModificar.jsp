        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Swal.fire('¡Perfecto!', 'El cliente fue añadido con éxito.', 'success').then(() => {
                  document.form.action = 'ActualizarCategoria';
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Swal.fire('¡Perfecto!', 'El cliente fue actualizado con éxito.', 'success').then(() => {
                    document.form.action = 'EliminarCategoria';
                    document.form.submit();
                });
            } else if (result.isDismissed) {
                Swal.fire('Cancelado.', 'Cancelaste la transacción :(', 'error').then(() => {
								...
								<form onsubmit="return false" name="form"  method="POST" >
									...
									<h5 class="text-condensedLight">Datos de la categoría</h5>
                                                                        <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
                                                                            <input class="mdl-textfield__input" type="number" name="txt_idcategoria" pattern="-?[A-Za-z0-9áéíóúÁÉÍÓÚ ]*(\.[0-9]+)?" id="idCategoria" value="<%=c.rs.getString("id_categorias")%>" readonly>
										<label class="mdl-textfield__label" for="NombreCategoría">ID (No podrás modificar este campo)</label>
										<span class="mdl-textfield__error">Nombre Inválido</span>
									</div>
									<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
                                                                            <input class="mdl-textfield__input" type="text" name="txt_nombrecategoria" pattern="-?[A-Za-z0-9áéíóúÁÉÍÓÚ ]*(\.[0-9]+)?" id="NombreCategoria" value="<%=c.rs.getString("nombre")%> ">
										<label class="mdl-textfield__label" for="NombreCategoría">Nombre</label>
										<span class="mdl-textfield__error">Nombre Inválido</span>
									</div>
									<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
                                                                            <input class="mdl-textfield__input" type="text" name="txt_descripcioncategoria" id="DescripcionCategoria"value="<%=c.rs.getString("descripcion")  %>">
									...
									<p class="text-center">
										<button class="mdl-button mdl-js-button mdl-button--fab mdl-js-ripple-effect mdl-button--colored bg-primary" id="btn-addCategory" onclick="verificar()" formaction="ActualizarCategoria">
											<i class="zmdi zmdi-plus"></i>
										</button>
                                                                                <button class="mdl-button mdl-js-button mdl-button--fab mdl-js-ripple-effect mdl-button--colored bg-danger" id="btn-deleteCategoria" onclick="verificarBorrar()" formaction="EliminarCategoria">
